    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                raise IndexError(f"Missing positional argument at index {index}")
            if not isinstance(val, expected_type):
                raise TypeError(f"Argument {index} to {func.__name__} must be {expected_type.__name__}, got {type(val).__name__}")
...
class MealyMachine:
    ...
    def transition(self, current_state, input_symbol, next_state, output_symbol):
        ...
        logger.debug(f"Added transition: {current_state} --{input_symbol}/{output_symbol}--> {next_state}")
        return self
    ...
    def process(self, inputs):
        ...
        for inp in inputs:
            ...
            match = next((t for t in self.transitions
                          if t.current_state == state and t.input_symbol == inp), None)
            if match is None:
                raise ValueError(f"No transition from state {state} on input {inp}")
            outputs.append(match.output_symbol)
            logger.info(f"Transition: {state} --{inp}/{match.output_symbol}--> {match.next_state}")
    ...
    def visualize_dot(self):
        ...
        lines = ["digraph Mealy {", "  rankdir=LR;", f"  label=\"{self.name}\";", ""]
        # Nodes
        ...
        for t in self.transitions:
            lines.append(f"  {t.current_state} -> {t.next_state} [label=\"{t.input_symbol}/{t.output_symbol}\"];  ")
        lines.append("}")
        dot = "\n".join(lines)
    ...
    def visualize_table(self):
        ...
        rows = [[t.current_state, t.input_symbol, t.output_symbol, t.next_state]
                for t in self.transitions]
        # Build markdown
        table = ["| " + " | ".join(header) + " |",
                 "|" + "---|" * len(header)]
